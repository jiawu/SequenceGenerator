...
7za a -t7z /projects/p20519/jia_output/SequenceGenerator/sequences_${motif_name}_${COUNTER}.7z /projects/p20519/jia_output/SequenceGenerator/${motif_name}_${COUNTER}*.txt
rm /projects/p20519/jia_output/SequenceGenerator/${motif_name}_${COUNTER}_*.txt
...
EOS
  ...
  sleep 25m
  jobs_running=true
  echo "jobs_are_running"
  if ((jobs_running)); then
    if [ -n $(qselect -u jjw036 -s QR)]; then
      echo "there are no jobs"
      scp /projects/p20519/jia_output/SequenceGenerator/sequences_${motif_name}_${COUNTER}.7z jia_wu@hera.chem-eng.northwestern.edu:/home/jia_wu/Public/SequenceGenerator/.
      jobs_running=false
    else
      echo "there are jobs"
      now=$(date + "%T")
